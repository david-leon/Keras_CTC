...
import theano
print('In models.py, theano.config.floatX=', theano.config.floatX)
    ...
    def compile(self, optimizer, loss, class_mode="categorical", sample_weight_mode=None, mask=False):
        ...
        if class_mode == 'ctc':
            ...
            if mask == True:
                ...
                self.y_mask = K.placeholder(ndim=2, dtype=self.y.dtype, name='y_mask')
                self.y_train_mask = K.placeholder(ndim=2, dtype=self.y_train.dtype, name='y_train_mask')
                self.y_test_mask = K.placeholder(ndim=2, dtype=self.y_train.dtype, name='y_test_mask')
            else:
                self.y_mask = None
                self.y_train_mask = None
                self.y_test_mask = None
        else:
            self.y = K.placeholder(ndim=K.ndim(self.y_train))                           # (B, T, C+1)
        ...
        if class_mode == 'ctc':
            train_loss = self.loss(self.y, self.y_train, self.y_mask, self.y_train_mask)
            test_loss = self.loss(self.y, self.y_test, self.y_mask, self.y_test_mask)
        else:
        ...
        if class_mode == "categorical":
            ...
            self.resultseq, self.resultseq_mask = objectives.ctc_decode(self.y_train)        # [DV] y_train: the scorematrix, (B, T, C+1)
            train_accuracy, train_TE, train_TD = objectives.ctc_CER(self.resultseq, self.y, self.resultseq_mask)

            self.resultseq_test, self.resultseq_mask_test = objectives.ctc_decode(self.y_test)
            test_accuracy, test_TE, test_TD = objectives.ctc_CER(self.resultseq_test, self.y,  self.resultseq_mask_test)
        else:
        ...
        if type(self.X_train) == list:
            ...
            if class_mode == 'ctc':
                if mask == False:
                    ...
                    train_ins = [self.X_train, self.y, self.y_train_mask, self.y_mask]
                    test_ins = [self.X_test, self.y, self.y_test_mask, self.y_mask]
            else:
                train_ins = [self.X_train, self.y, self.weights]
                test_ins = [self.X_test, self.y, self.weights]
    ...
    def train_on_batch(self, X, y, X_mask=None, y_mask=None, accuracy=False, class_weight=None, sample_weight=None):
        ...
        if self.class_mode == 'ctc':
            if X_mask is None or y_mask is None:
                ...
                ins = X + [y, X_mask, y_mask]

        else:
            sample_weight = standardize_weights(y, class_weight=class_weight, sample_weight=sample_weight, sample_weight_mode=self.sample_weight_mode)
            ins = X + [y, sample_weight]
